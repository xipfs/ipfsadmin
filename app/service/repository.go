...
// 返回一个仓库对象
func (this *repositoryService) GetRepoByResourceId(resourceId int) (*Repository, error) {
	resource, err := ResourceService.GetResource(resourceId)
	if err != nil {
		return nil, err
	}
	return OpenRepository(resource.Domain)
}

// 获取某资源库的标签列表
func (this *repositoryService) GetTags(resourceId int, limit int) ([]string, error) {
	repo, err := this.GetRepoByResourceId(resourceId)
	if err != nil {
		return nil, err
	}
	repo.Pull()
...
func OpenRepository(repoPath string) (*Repository, error) {
	repoPath = GetResourcePath(repoPath)
