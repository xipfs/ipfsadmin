...
func (this *actionService) format(action *entity.Action) {
	...
	case "pub_task":
		...
		if err != nil {
			...
			action.Message = fmt.Sprintf("<b>%s</b> 发布了 <span class='blue'>%s</span> 的 <b>%s</b> 版本。", action.Actor, task.ResourceInfo.Name, task.ResourceInfo.Version)
		}
	case "del_task":
	...
	case "review_task":
		...
		if err != nil {
			...
			if action.Extra == "1" {
				action.Message = fmt.Sprintf("<b>%s</b> 审批了 <span class='text-primary'>%s</span> 编号为<b>%d</b>的发布单，结果为：<b class='green'>通过</b>", action.Actor, task.ResourceInfo.Name, action.ObjectId)
			} else if action.Extra == "-1" {
				action.Message = fmt.Sprintf("<b>%s</b> 审批了 <span class='text-primary'>%s</span> 编号为<b>%d</b>的发布单，结果为：<b class='red'>不通过</b>", action.Actor, task.ResourceInfo.Name, action.ObjectId)
			}
		}
	}
}
