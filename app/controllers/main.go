...
func (this *MainController) Index() {
	...
	for k, stat := range resourcesStat {
		resourceInfo, err := service.ResourceService.GetResource(stat["resource_id"])
		if err != nil {
			continue
		}
		if k > 4 {
		...
		info["resource_name"] = resourceInfo.Name
		info["version"] = resourceInfo.Version
		info["version_time"] = beego.Date(resourceInfo.VersionTime, "Y-m-d H:i:s")
		info["count"] = stat["count"]
		popResources = append(popResources, info)
	}
	...
	this.Data["peerNum"], _ = service.PeerService.GetTotal()
	this.Data["resourceNum"], _ = service.ResourceService.GetTotal()
	this.Data["pubNum"], _ = service.TaskService.GetPubTotal()
	this.Data["popResources"] = popResources
